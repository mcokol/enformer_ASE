...
ax.plot(xv, poly1d_fn(xv), color="green", linewidth=2, label="Linear fit")
...
stats_text = (
    ...
    f"isCoding = {iscoding} ({iscodingpercent:.1f}%)\n"
    f"Linear: green\n"
    f"Lowess: magenta"
)
ax.text(
...
ax_histx.plot(bin_centers, counts_0n, drawstyle="steps-mid", color="orange", label="0 RPKM, non-coding")
ax_histx.plot(bin_centers, counts_pc, drawstyle="steps-mid", color="blue",   label=">0 RPKM, coding")
ax_histx.plot(bin_centers, counts_pn, drawstyle="steps-mid", color="green",  label=">0 RPKM, non-coding")
